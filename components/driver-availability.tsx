...
export function DriverAvailability() {
  ...
  const [availability, setAvailability] = useState<
    Record<string, "available" | "unavailable" | "booked">
  >({});
  ...
  useEffect(() => {
      ...
      fetch("/api/driver/availability")
        ...
        .then((data) => {
          if (data.availability) {
            const availObj: Record<
              string,
              "available" | "unavailable" | "booked"
            > = {};
            data.availability.forEach(
              (item: { date: string; status: "unavailable" | "booked" }) => {
    ...
    load();
    const handler = () => load();
    window.addEventListener("driver:availabilityUpdated", handler);
    return () =>
      window.removeEventListener("driver:availabilityUpdated", handler);
  }, []);
  ...
  const toggleAvailability = (dateStr: string) => {
    ...
    const unavailableDates = Object.keys(updatedAvailability).filter(
      (date) => updatedAvailability[date] === "unavailable"
    );
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Automatically accept booking requests for available days
              </p>
            </div>
            <Switch
              id="auto-accept"
      ...
      <Card>
        ...
        <CardContent>
            ...
            {days.map((day, index) => {
              ...
              return (
                <Button
                  ...
                  size="sm"
                  className={cn(
                    "h-12 text-sm transition-colors bg-white",
          ...
          <div className="mt-6 p-4 bg-muted/30 rounded-lg">
            <p className="text-sm text-muted-foreground">
              <strong>Tip:</strong> Click on any available or unavailable day to
              toggle your availability. Booked days cannot be changed.
            </p>
          </div>
        </CardContent>
      </Card>
