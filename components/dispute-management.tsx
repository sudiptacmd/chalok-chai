...
interface TicketMessage { senderUserId: string; message: string; createdAt: string }
type Priority = "low"|"medium"|"high"|"urgent";
interface Ticket {
  _id: string; subject: string; status: "open"|"pending"|"resolved"|"closed";
  priority?: Priority; createdAt: string; lastMessageAt: string;
  messages: TicketMessage[]; createdByUserId: { _id: string; name?: string; email: string; type: string };
...
export const DisputeManagement: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/tickets${qs.toString() ? `?${qs}` : ""}`);
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-muted-foreground">
                        ...
                        <span className={chipFor(t.priority)}>{t.priority || "medium"}</span>
          ...
          <div className="h-full grid grid-rows-[auto_1fr_auto] gap-4">
            ...
            <div className="rounded-xl border bg-card/60 backdrop-blur p-4 sticky top-0 z-10 space-y-2">
              <div className="flex flex-wrap items-center gap-3">
                <h2 className="text-base font-semibold mr-auto">{active.subject}</h2>
                <Select value={active.status} onValueChange={(v)=>updateStatus(v as Ticket["status"])}>
                ...
                <span className={chipFor(active.priority)}>{active.priority || "medium"}</span>
              </div>
              <div className="flex flex-wrap gap-3 text-[11px] text-muted-foreground">
                <span><strong>Reporter:</strong> {active.createdByUserId?.name || active.createdByUserId?.email}</span>
            ...
            <div className="space-y-3 overflow-auto pr-1">
              {active.messages.map((m, i) => {
                const isAdmin = m.senderUserId === "admin" || m.senderUserId === "staff"; 
                return (
                  <div key={i} className={`flex ${isAdmin ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[80%] rounded-2xl border p-3 text-sm shadow-sm ${
                      isAdmin ? "bg-primary/10 border-primary/20" : "bg-muted/40 border-border/50"
                    }`}>
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  ...
                  <div className="mt-1 flex justify-between text-[11px] text-muted-foreground">
                    ...
                    <span>{reply.length}/{MSG_MAX}</span>
                  </div>
                </div>
                <Button className="h-10" disabled={!reply.trim() || loading} onClick={sendReply}>
