...
import { RefreshCcw, Search, Filter, ArrowUpDown } from "lucide-react";
...
export const ManageTickets: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      setRefreshing(true);
      const res = await fetch("/api/tickets?scope=involved", { cache: "no-store" });
  ...
  const markRead = useCallback(async (id: string) => {
    try { await fetch(`/api/tickets/${id}/read`, { method: 'POST' }); } catch {}
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(t => t.subject.toLowerCase().includes(q) || t.messages.some(m => m.message.toLowerCase().includes(q)));
    }
    if (sort === 'recent') {
      ...
      list.sort((a,b)=> (order[a.priority||'medium'] ?? 5) - (order[b.priority||'medium'] ?? 5));
    }
    return list;
  ...
  const priorityBadgeColor = (p?: string) => {
    switch (p) {
      case 'urgent': return 'bg-red-600/20 text-red-700';
      case 'high': return 'bg-orange-500/20 text-orange-600';
      case 'low': return 'bg-gray-400/20 text-gray-600';
    ...
    <div className="flex gap-6">
      <div className="w-80 space-y-3 border-r pr-3">
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="icon" onClick={load} disabled={refreshing} className="h-9 w-9">
        ...
        <div className="space-y-2 max-h-[calc(100vh-340px)] overflow-auto pr-1">
          {filtered.map(t => (
            <button key={t._id} onClick={()=> setActiveId(t._id)} className={`group relative block w-full text-left px-3 py-2 rounded border transition ${activeId===t._id ? 'bg-accent/40 border-accent' : 'hover:bg-muted/50'}`}>
              <div className="flex items-center justify-between gap-2 text-xs mb-0.5">
                <span className="font-medium truncate max-w-[150px]">{t.subject}</span>
                <span className={`px-1.5 py-0.5 rounded capitalize text-[10px] ${priorityBadgeColor(t.priority)}`}>{t.priority||'medium'}</span>
              </div>
          ...
          {!filtered.length && <p className="text-xs text-muted-foreground px-1">No tickets found.</p>}
        </div>
      </div>
      <div className="flex-1">
        {!active && <p className="text-sm text-muted-foreground">Select a ticket</p>}
        {active && (
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              {active.relatedBookingId && (
                <span className="text-xs text-muted-foreground">Trip: {active.relatedBookingId.bookingType} • {active.relatedBookingId.status} {active.relatedBookingId.startDate ? '• '+ new Date(active.relatedBookingId.startDate).toLocaleDateString(): ''}</span>
              )}
              <span className="text-xs uppercase tracking-wide">{active.status}</span>
            </div>
            <div className="flex justify-end">
              <Button variant="outline" size="sm" onClick={()=> activeId && load()} className="h-8 text-xs gap-1"><ArrowUpDown className="h-3 w-3" /> Refresh</Button>
            </div>
