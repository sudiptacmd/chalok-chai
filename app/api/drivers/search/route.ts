...
export async function GET(req: NextRequest) {
	try {
		...
		const bookingType = searchParams.get("bookingType") === "monthly" ? "monthly" : "daily";
		...
		const total = await Driver.countDocuments(query);
				...
				interface PopulatedDriverDoc {
					_id: { toString(): string };
					userId?: { name?: string; profilePhoto?: string; emailVerified?: boolean };
			...
			const results = (drivers as unknown as PopulatedDriverDoc[]).map((d) => ({
				...
				reviewCount: d.ratings?.length || 0,
						experience: d.experienceYears && d.experienceYears >= 5 ? "5+ years" : d.experienceYears && d.experienceYears >= 3 ? "3-5 years" : d.experienceYears && d.experienceYears >= 1 ? "1-2 years" : "<1 year",
				location: d.location,
				...
				bio: d.bio || "",
						availability: (d.availability || []).reduce<Record<string, string>>((acc, slot) => {
