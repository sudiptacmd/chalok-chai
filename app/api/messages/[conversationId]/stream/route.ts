...
export async function GET(_req: NextRequest, { params }: { params: { conversationId: string } }) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const listener = (event: { type: "message"; conversationId: string; messageId: string }) => send(event);
      conversationHub.on(params.conversationId, listener);
      ...
      try {
        ...
        const pipeline = [
          { $match: { operationType: "insert", "fullDocument.conversationId": conv._id } },
        ];
        changeStream = coll.watch(pipeline as any, { fullDocument: "updateLookup" });
        changeStream.on("change", (change: any) => {
          const msgId = change.fullDocument?._id?.toString();
          if (msgId) {
            send({ type: "message", conversationId: conv._id.toString(), messageId: msgId });
          }
        });
      } catch (err) {
      ...
      const cleanup = () => {
        ...
        closed = true;
        clearInterval(ping);
        conversationHub.off(params.conversationId, listener);
