...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (user.type === 'driver') {
        return NextResponse.redirect(
          new URL('/signin?message=Email verified successfully. Your account is pending admin approval.', request.url)
        )
      } else {
        return NextResponse.redirect(
          new URL('/signin?message=Email verified successfully. You can now sign in.', request.url)
        )
      }
      ...
      return NextResponse.redirect(
        new URL('/signin?error=Invalid or expired verification link', request.url)
      )
    }
