...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate input
    if (!name || !email || !phone || !dateOfBirth || !nationalId || 
        !drivingLicenseNumber || !location || !password || !confirmPassword) {
    ...
    if (drivingLicensePhoto && drivingLicensePhoto.size > 0) {
      const licensePhotoBuffer = Buffer.from(await drivingLicensePhoto.arrayBuffer())
    ...
    const actualVerificationToken = generateEmailVerificationToken(savedUser._id.toString())
    ...
    return NextResponse.json({
      message: 'Driver account created successfully. Please check your email to verify your account. Your account will be reviewed by an administrator.',
      userId: savedUser._id
    })
